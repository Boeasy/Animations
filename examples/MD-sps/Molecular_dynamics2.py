...
class MD_T(Scene):
    """Enhanced version with Lennard-Jones potential graph"""
    def construct(self):
        ...
        # Create Lennard-Jones potential graph
        graph_axes = Axes(
            x_range=[0.8, 3.5, 0.5],
            y_range=[-1.5, 2, 0.5],
            x_length=3.5,
        ...
        # Create Lennard-Jones potential curve
        lj_curve = graph_axes.plot(
        ...
        graph_title = Text("L-J Potential", font_size=24).next_to(graph_axes, UP, buff=0.2)
        ...
        title = Text("MD Simulation with L-J Potential", font_size=28)
        title.to_edge(UP)
        
        # Show setup
        self.add(boundary, title, particles, particle_line)
        self.add(graph_axes, lj_curve, graph_title, x_label, y_label, indicator_dot, distance_text)
